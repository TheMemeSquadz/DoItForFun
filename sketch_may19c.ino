const int a = 8; //For displaying segmant "a"
...
void checkIncButtonPress()
{
  ...
  if (IncbuttonState != lastIncbuttonState) {
     // if the current state has changed, increment the counter
     if (IncbuttonState == LOW) {
      // if the current state is HIGH then the button went off to on
      bPress = true;
      buttonPushCounter++;
      if( buttonPushCounter > 9) butttonPushCounter =0 ;
      Serial.printIn("on");
     } else {
  ...
  // save the current state as the last state, for next through the loop
  lastDecbuttonState = Decbutton State;
}
...
void displayDigit(int digit)
{
 ...
 if(digit !=1 && digit 1=2 && digit!=3 && !=7)
 digitalWrite(f,HIGH);
 if (digit!=0 && digit !=7)
 digitalWrite(g,HIGH);
}
